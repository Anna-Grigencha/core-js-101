...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  if (isEndIncluded === true) { rightBracket = ']'; } else {
    rightBracket = ')';
  }
  if (a <= b) {
    leftNum = a;
...
function isCreditCardNumber(ccn) {
  let sum = 0;
  const ccnRevers = ccn.toString().split('').reverse().join('');
  for (let i = 1; i < ccnRevers.length; i += 1) {
    if (i % 2 === 0) {
      sum += (+ccnRevers[i]);
    } else if (+ccnRevers[i] > 4) {
      sum = sum + 2 * ccnRevers[i] - 9;
    } else {
      sum += 2 * ccnRevers[i];
    }
  }
...
function getDigitalRoot(num) {
  ...
  if (sum1 >= 10) {
    const arr2 = sum1.toString().split('');
    const sum2 = arr2.reduce((sum, current) => sum + (+current), 0);
    return sum2;
  }
  return sum1;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  const splitedPathes = pathes.map((element) => element.split('/'));

  const maxPathLenth = Math.min(...splitedPathes.map((el) => el.length));

  for (let i = 0; i < maxPathLenth; i += 1) {
    const value = splitedPathes[0][i];

    if (splitedPathes.every((el) => el[i] === value)) {
      commonPath += `${value}/`;
    }
  }
